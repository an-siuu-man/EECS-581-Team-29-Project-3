...
export async function POST(req: Request) {
  try {
    ...
    const resolveClassId = async (uuid?: string, classId?: number) => {
      ...
      if (error) throw new Error(`Failed to look up class by uuid: ${error.message}`);
    ...
    if (fromClassId == null) {
      return Response.json({ error: 'Source class not found' }, { status: 404 });
    }
    if (toClassId == null) {
      return Response.json({ error: 'Target class not found' }, { status: 404 });
    }
    ...
    const { error: upsertErr } = await supabase
      ...
      .upsert(
        [{ scheduleid, classid: toClassId }],
        { onConflict: 'scheduleid,classid', ignoreDuplicates: false }
      );

    if (upsertErr) {
      console.error('[replaceClass] upsert error:', upsertErr);
      return Response.json({ error: 'Failed to add target class to schedule' }, { status: 500 });
    }
