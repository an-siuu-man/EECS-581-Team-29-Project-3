...
const ActiveScheduleContext = createContext<ActiveScheduleContextType | undefined>(undefined);

export const ActiveScheduleProvider = ({ children }: { children: ReactNode }) => {
  const { user } = useAuth();
  ...
  const [activeSchedule, setActiveSchedule] = usePersistedState<Schedule | null>("activeSchedule", null);
  
  // Active semester filter
  const [activeSemester, setActiveSemester] = usePersistedState<string>("activeSemester", "");
  
  // All user schedules
  const [userSchedules, setUserSchedules] = usePersistedState<Schedule[]>("userSchedules", []);
  ...
  const updateScheduleInList = (scheduleId: string, updatedSchedule: Schedule) => {
...
export const useActiveSchedule = () => {
  ...
  if (context === undefined) {
    throw new Error('useActiveSchedule must be used within an ActiveScheduleProvider');
  }
  return context;
};
